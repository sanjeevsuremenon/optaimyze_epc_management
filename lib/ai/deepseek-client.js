...
export async function callDeepSeekChat(messages, options = {}) {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (!response.ok) {
        ...
        const err = new Error(`LLM API error (${response.status}): ${errorText.slice(0, 300)}`);
      ...
      if (!content) {
        const err = new Error('Empty response received from LLM model');
        err.isLlmError = true;
        throw err;
      }
  ...
  const finalError = lastError || new Error('Failed to communicate with LLM API');
  finalError.isLlmError = true;
  throw finalError;
}
