  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/poschedule/lc', {
        ...
        body: JSON.stringify({
          ponumber,
          lcdata: formattedData
        }),
      });

      const data = await response.json();
      if (data.success) {
        alert('LC data saved successfully!');
      } else {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 flex">
      ...
      <div 
        ...
        className="bg-amber-600 text-white px-2 py-4 rounded-l-lg cursor-pointer hover:bg-amber-700 transition-colors duration-150 flex items-center"
      >
      ...
      <div className="flex-1">
        <div 
          onClick={() => setIsExpanded(!isExpanded)}
          className="p-3 flex justify-end cursor-pointer hover:bg-amber-50 dark:hover:bg-gray-700 transition-colors duration-150"
        >
          <svg 
          ...
          {isLoading ? (
            ...
            <form onSubmit={handleSubmit} className="p-4">
              <div className="max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-lg overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {formFields.map((field, index) => (
                    <div 
                      key={field.name} 
                      className={`
                        relative p-4 bg-amber-50/50 dark:bg-gray-800/50 
                        rounded-lg border border-amber-100 dark:border-gray-700
                        hover:shadow-md transition-shadow duration-200
                        ${field.type === 'textarea' ? 'md:col-span-2 lg:col-span-3 xl:col-span-4' : ''}
                        ${index % 2 === 0 ? 'bg-amber-50/30' : 'bg-amber-50/60'}
                      `}
                    >
                      <label 
                        htmlFor={field.name}
                        className="block text-sm font-bold text-amber-900 dark:text-amber-300 mb-2"
                      >
                        {field.label}
                      </label>
                      {field.type === 'textarea' ? (
                        <textarea
                          ...
                          rows="2"
                          className={`block w-full px-3 py-2 rounded-md border border-amber-200 dark:border-gray-600 
                                   shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent
                                   bg-white dark:bg-gray-700 
                                   ${formData[field.name] 
                                     ? 'text-amber-900 font-semibold dark:text-amber-300' 
                        ...
                        <div className="relative">
                          ...
                          {field.type === 'date' && (
                            <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                              <svg 
                                className={`h-5 w-5 ${formData[field.name] ? 'text-amber-600' : 'text-gray-400'}`} 
                                fill="currentColor" 
                                viewBox="0 0 20 20"
                              >
                ...
                <div className="mt-6 border-t border-amber-200 dark:border-gray-700 pt-4">
