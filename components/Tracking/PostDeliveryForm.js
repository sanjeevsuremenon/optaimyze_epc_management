  ...
  useEffect(() => {
    if (debouncedPoReferenceInput && debouncedPoReferenceInput.length >= 3) {
      fetch(`/api/purchaseorders/search?str=${debouncedPoReferenceInput}`)
        .then(res => res.json())
        .then(data => {
          setPoReferenceSuggestions(Array.isArray(data) && data.length ? data : []);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-lg bg-app-bg text-app-text ${errors.poReference ? 'border-red-500' : 'border-app-border'}`}
              required
              autoComplete="off"
          ...
          <div>
            ...
            <input type="text" name="inspectionReference" value={formData.inspectionReference} onChange={e => setFormData(prev => ({ ...prev, inspectionReference: e.target.value }))} className={`w-full px-3 py-2 border rounded-lg bg-app-bg text-app-text ${errors.inspectionReference ? 'border-red-500' : 'border-app-border'}`} required />
