...
const VENDOR_SEARCH_DEBOUNCE_MS = 400;

function normalizeVendorList(data) {
  if (Array.isArray(data)) return data;
  if (data && Array.isArray(data.vendors)) return data.vendors;
...
export default function VendorFeedbackPage() {
  ...
  // Vendor list (up to 100) + search
  const [vendorFeedbackSearchTerm, setVendorFeedbackSearchTerm] = useState('');
  const [vendorList, setVendorList] = useState([]);
  const [vendorListTotal, setVendorListTotal] = useState(0);
  const [vendorListLoading, setVendorListLoading] = useState(false);
  const [selectedVendorForFeedback, setSelectedVendorForFeedback] = useState(null);
  ...
  // Load vendors (limit 100); search when term present
  useEffect(() => {
    if (!session) return;
    const timer = setTimeout(async () => {
      ...
      try {
        const params = new URLSearchParams({
          limit: String(VENDOR_LIST_LIMIT),
          page: '1',
        });
        if (term) params.set('str', term);
        const res = await fetch(`/api/vendors?${params.toString()}`);
        const data = await res.json().catch(() => ({}));
        const list = normalizeVendorList(data);
        setVendorList(list);
        setVendorListTotal(
  ...
  }, [session, vendorFeedbackSearchTerm]);
  ...
  const selectVendorFromList = (vendor) => {
    const code = vendor['vendor-code'] ?? vendor.vendorcode ?? vendor.vendorCode ?? '';
    const name = vendor['vendor-name'] ?? vendor.vendorname ?? vendor.vendorName ?? '';
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      ...
      try {
        const response = await fetch(`/api/vendors?str=${encodeURIComponent(searchTerm)}&limit=20`);
  ...
  if (status === 'loading') {
    return <div className="app-page min-h-screen flex items-center justify-center text-app-text">Loading...</div>;
  }
        ...
        <div className="flex flex-col">
          ...
          <div className="flex-1">
            <div className="max-w-7xl mx-auto px-4 py-6 pb-16 mb-24 w-full flex flex-col gap-6">
                ...
                <div className="mt-4 overflow-x-auto border border-app-border rounded-xl">
                  <table className="min-w-full divide-y divide-app-border">
                    <thead className="bg-app-surface-muted">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-bold text-app-text-secondary uppercase tracking-wider">Vendor Code</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-app-text-secondary uppercase tracking-wider">Vendor Name</th>
                        <th className="px-4 py-3 text-right text-xs font-bold text-app-text-secondary uppercase tracking-wider">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-app-border bg-app-surface">
                      {vendorListLoading && vendorList.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="px-4 py-10 text-center text-app-text-muted text-sm">
                            Loading vendors…
                          </td>
                        </tr>
                      ) : vendorList.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="px-4 py-10 text-center text-app-text-muted text-sm">
                            {vendorFeedbackSearchTerm.trim()
                              ? `No vendors found matching "${vendorFeedbackSearchTerm.trim()}".`
                              : 'No vendors found.'}
                          </td>
                        </tr>
                      ) : (
                        vendorList.map((v, idx) => {
                          const code = v['vendor-code'] ?? v.vendorcode ?? v.vendorCode ?? '';
                          const name = v['vendor-name'] ?? v.vendorname ?? v.vendorName ?? '';
                          const isSelected = selectedVendorForFeedback?.vendorCode === code;
                          return (
                            <tr
                              key={code ? `${code}-${idx}` : idx}
                              className={`transition-colors cursor-pointer ${
                                isSelected
                                  ? 'bg-app-accent-soft/40'
                                  : 'hover:bg-app-surface-muted/70'
                              }`}
                              onClick={() => selectVendorFromList(v)}
                            >
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-right">
                                <button
                                  ...
                                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-app-accent text-app-accent-text text-xs font-semibold hover:bg-app-accent-hover transition"
                                >
                                  <FontAwesomeIcon icon={faStar} className="w-3 h-3" />
                                  {isSelected ? 'Selected' : 'Give feedback'}
                                </button>
                              </td>
                            </tr>
                          );
                ...
                <div className="p-6 bg-app-surface border border-app-border rounded-2xl shadow-sm">
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="text-base font-bold text-app-text">
                      Feedback for: {selectedVendorForFeedback.vendorName || selectedVendorForFeedback.vendorCode}
                    </h3>
                    <button
                      type="button"
                  ...
                  <div className="flex flex-col gap-4 w-full mb-5">
                    <div className="bg-app-bg rounded-lg px-3 py-1.5 border border-app-border self-start text-xs text-app-text-muted">
                      <span className="font-bold text-app-text-muted uppercase tracking-wider text-[10px]">Vendor Code:</span>{' '}
                      <span className="font-bold text-app-accent ml-1">{selectedVendorForFeedback.vendorCode}</span>
                    </div>

                    {subgroups.length > 0 && (
                      <div className="w-full">
                        <span className="block text-[10px] text-app-text-muted font-bold uppercase tracking-wider mb-2">
                          Subgroups Mapped ({subgroups.length})
                        </span>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 w-full">
                          {subgroups.map((sg, idx) => {
                            const bgClass = idx % 2 === 0 ? 'bg-app-bg/60 hover:bg-app-bg' : 'bg-app-surface-muted hover:bg-app-surface';
                            return (
                              <div
                                key={idx}
                                className={`px-4 py-2.5 rounded-xl border border-app-border flex flex-col gap-1 transition-all ${bgClass}`}
                              >
                                <div className="flex justify-between items-center">
                                  <span className="text-[9px] font-black text-app-accent uppercase tracking-widest leading-none">{sg.groupName}</span>
                                  {sg.isService ? (
                                    <span className="text-[8px] font-bold uppercase tracking-wider text-violet-600 dark:text-violet-300 bg-violet-500/15 border border-violet-500/25 px-1.5 py-0.5 rounded">
                                      Service
                                    </span>
                                  ) : (
                                    <span className="text-[8px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-500/15 border border-emerald-500/25 px-1.5 py-0.5 rounded">
                                      Material
                                    </span>
                                  )}
                                </div>
                  ...
                  <div className="space-y-4">
                    {userHasAlreadySubmitted && (
                      <p className="p-3.5 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-700 dark:text-amber-400 text-xs leading-normal">
                    ...
                    <div>
                      ...
                      />
                      <div className="flex flex-col sm:flex-row gap-4 mt-3 p-3 bg-app-bg border border-app-border rounded-lg">
                        {isMaterialDisabled && (
                          <label className="flex items-center gap-2 text-xs font-semibold text-app-text-muted cursor-pointer hover:text-app-text-secondary transition-colors">
                            <input
                              ...
                              disabled={userHasAlreadySubmitted}
                              className="rounded text-app-accent focus:ring-app-accent bg-app-surface border-app-border w-4 h-4 cursor-pointer"
                            />
                    ...
                    <button
                      ...
                      className="bg-app-accent hover:bg-app-accent-hover disabled:opacity-50 text-app-accent-text px-4 py-2 rounded-lg flex items-center gap-1.5 transition-all text-xs font-bold shadow-sm uppercase tracking-wider"
                  ...
                  {materialFeedbackLoading ? (
                    ...
                    <>
                      <div className="p-3.5 bg-app-bg border border-app-border rounded-lg mb-4 text-xs flex justify-between flex-wrap gap-4">
                        <p className="mb-0">
                          <strong>Average rating:</strong>{' '}
                          <span className="text-amber-600 dark:text-amber-400 ml-1 font-bold">
                            {materialAverageRating > 0 ? `${materialAverageRating} / 5` : 'No ratings yet'}
                          </span>
                          {materialFeedbacks.length > 0 && (
                            <span className="text-app-text-muted ml-2">
                              ({materialFeedbacks.length} rating{materialFeedbacks.length !== 1 ? 's' : ''})
                            </span>
                          )}
                        </p>
                        <p className="mb-0">
                          <strong>Tier votes:</strong> Top: {materialTierVotes.top} · Middle: {materialTierVotes.middle} · Lower: {materialTierVotes.lower}
                        </p>
                      </div>
                      {materialFeedbacks.length === 0 ? (
                          ...
                          {materialFeedbacks.map((f) => (
                            <div key={f._id} className="p-4 bg-app-surface-muted/80 border border-app-border rounded-xl text-xs flex flex-col gap-2">
                              ...
                              {(() => {
                                ...
                                return (
                                  <p className="text-amber-600 dark:text-amber-400 font-bold text-xs mb-0">
              ...
              {showForm && (
                <div className="mb-6 p-6 bg-app-surface border border-app-border rounded-2xl shadow-sm">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      {showVendorResults && vendors.length > 0 && (
                        <div className="mt-2 bg-app-surface border border-app-border rounded-md shadow-lg max-h-60 overflow-y-auto divide-y divide-app-border">
                          {vendors.map((vendor, index) => (
                            <div
                              key={index}
                              onClick={() => handleVendorSelect(vendor)}
                              className="px-4 py-3 hover:bg-app-surface-muted cursor-pointer"
                            >
                      ...
                      {showVendorResults && vendors.length === 0 && !vendorSearchLoading && searchTerm && (
                        <div className="mt-2 bg-app-bg border border-app-border rounded-md p-4 text-center text-app-text-muted text-xs">
                    ...
                    <div className="flex gap-2">
                      <button
                        type="submit"
                        className="bg-app-accent hover:bg-app-accent-hover text-app-accent-text px-4 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm"
                      >
                        <FontAwesomeIcon icon={faSave} />
              ...
              {/* Legacy comment feedbacks */}
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <h2 className="text-base font-bold text-app-text">Comment feedbacks</h2>
                  <div className="relative max-w-sm w-full">
                    <input
                      type="text"
                      value={feedbackSearchTerm}
                      onChange={(e) => setFeedbackSearchTerm(e.target.value)}
                      placeholder="Search comment feedbacks…"
                      className="w-full px-3 py-2 pl-10 bg-app-bg border border-app-border rounded-lg focus:outline-none focus:border-app-accent text-app-text text-xs focus:ring-1 focus:ring-app-accent placeholder:text-app-text-disabled"
                    />
                    <FontAwesomeIcon icon={faSearch} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-app-text-muted" />
                  </div>
                </div>
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    <div className="inline-block bg-app-surface border border-app-border rounded-xl shadow-sm p-6">
                      <div className="text-app-text-muted text-sm">
                        {feedbackSearchTerm
                          ? `No feedbacks found matching "${feedbackSearchTerm}"`
                          : 'No comment feedbacks yet.'}
                      </div>
                    </div>
                  </div>
                ) : (
                            ...
                            {(feedback.vendorCode || feedback.vendorName) && (
                              <div className="mb-4 p-3 bg-app-bg rounded-lg border border-app-border">
                                <div className="text-xs text-app-text-muted">
                                  {feedback.vendorCode && (
                                    <div className="mb-1">
                                      <span className="font-bold text-app-accent text-[10px] uppercase tracking-wider">Vendor Code:</span>
                                      <span className="ml-2 text-app-text-secondary font-semibold">{feedback.vendorCode}</span>
                                    </div>
                                  )}
                                  {feedback.vendorName && (
                                    <div>
                                      <span className="font-bold text-app-accent text-[10px] uppercase tracking-wider">Vendor Name:</span>
