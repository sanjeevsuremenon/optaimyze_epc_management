...
export default function AssetDetailsPage() {
  ...
  if (error || !asset) {
    return (
      <div className="flex justify-center items-center h-screen bg-slate-950 text-white">
        <div className="bg-slate-900 border border-red-500/50 p-6 rounded-xl shadow-xl">
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-start bg-slate-900/60 rounded-xl p-6 border border-slate-700 shadow-xl">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">{asset.assetnumber}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg">
          <h2 className="text-xl font-bold text-cyan-400 border-b border-slate-700 pb-2 mb-4">Core Information</h2>
          <dl className="grid grid-cols-1 gap-y-4">
            <div>
              <dt className="text-sm font-semibold text-slate-400">Category</dt>
              <dd className="mt-1 text-base text-slate-100">{asset.assetcategory || '—'}</dd>
            </div>
            <div>
              <dt className="text-sm font-semibold text-slate-400">Subcategory</dt>
              <dd className="mt-1 text-base text-slate-100">{asset.assetsubcategory || '—'}</dd>
            </div>
            <div>
              <dt className="text-sm font-semibold text-slate-400">Status</dt>
              <dd className="mt-1">
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg">
          <h2 className="text-xl font-bold text-cyan-400 border-b border-slate-700 pb-2 mb-4">Acquisition Details</h2>
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-4">
            <div>
              <dt className="text-sm font-semibold text-slate-400">Acquisition Date</dt>
              <dd className="mt-1 text-base text-slate-100">
                {asset.acquireddate ? new Date(asset.acquireddate).toLocaleDateString() : '—'}
              </dd>
            </div>
            <div>
              <dt className="text-sm font-semibold text-slate-400">Acquisition Value</dt>
            ...
            <div>
              <dt className="text-sm font-semibold text-slate-400">Manufacturer</dt>
              <dd className="mt-1 text-base text-slate-100">{asset.assetmanufacturer || '—'}</dd>
            </div>
            <div>
              <dt className="text-sm font-semibold text-slate-400">Model</dt>
              <dd className="mt-1 text-base text-slate-100">{asset.assetmodel || '—'}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-sm font-semibold text-slate-400">Serial Number</dt>
              <dd className="mt-1 text-base text-slate-100">{asset.assetserialnumber || '—'}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg md:col-span-2">
          <div className="flex justify-between items-center border-b border-slate-700 pb-2 mb-4">
            <h2 className="text-xl font-bold text-cyan-400">Custody History</h2>
          ...
          {custodyRecords.length > 0 ? (
              ...
              <table className="table w-full text-left">
                ...
                <tbody className="divide-y divide-slate-700">
                  {custodyRecords.map((c, i) => (
                    <tr key={c._id || i} className="text-slate-200">
                      <td className="px-4 py-2 align-top">
                        <div className="font-medium text-slate-100">{c.employeename || 'N/A'}</div>
                        {c.employeenumber && <div className="text-slate-400 text-xs">ID: {c.employeenumber}</div>}
                        {c.custodianDetail && <div className="text-slate-400 text-xs mt-1">{c.custodianDetail}</div>}
                      </td>
                      <td className="px-4 py-2 align-top max-w-xs break-words">
                        ...
                        {c.custodyRemark && <div className="text-slate-500 text-xs mt-1 italic">Note: {c.custodyRemark}</div>}
                      </td>
                      ...
                      <td className="px-4 py-2 align-top text-sm font-mono text-slate-300">
                        {c.documentnumber || '—'}
                      </td>
                    </tr>
                  ))}
            ...
            <p className="text-slate-400 text-sm">No custody records found.</p>
          )}
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg md:col-span-2">
          <div className="flex justify-between items-center border-b border-slate-700 pb-2 mb-4">
            <h2 className="text-xl font-bold text-cyan-400">Calibration Certificates</h2>
          ...
          {calibrations.length > 0 ? (
            calibrations.every(c => c.calibrationRequired === 'Not Required') ? (
              <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/50 text-center">
                ...
                <p className="text-slate-500 text-sm mt-1">This asset is marked as not requiring calibration.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="table w-full text-left">
            ...
            <p className="text-slate-400 text-sm">No calibration records found.</p>
          )}
        </div>

        {asset.accessories && (
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg md:col-span-2">
            <h2 className="text-xl font-bold text-cyan-400 border-b border-slate-700 pb-2 mb-4">Accessories</h2>
            <div 
        ...
        {asset.assetnotes && (
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg md:col-span-2">
            <h2 className="text-xl font-bold text-cyan-400 border-b border-slate-700 pb-2 mb-4">Notes</h2>
            <div 
              className="prose prose-invert max-w-none text-slate-300"
