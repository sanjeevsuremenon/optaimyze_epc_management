...
export default function VendorEvaluationDetails() {
  ...
  if (error) {
    return (
      <div className="app-page min-h-screen flex flex-col items-center justify-center gap-4">
        <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-6 max-w-md text-center shadow-xl">
          ...
          <p className="text-xs text-app-text-muted leading-normal">{error}</p>
        </div>
        <Link href="/vendorevaluation/webformat" className="flex items-center gap-1.5 text-xs text-app-accent hover:underline">
        ...
        <div className="min-h-screen pb-16">
          ...
          <div className="flex items-center justify-between">
            <Link 
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-app-surface border border-app-border rounded-lg text-xs font-semibold text-app-text-secondary hover:text-app-text hover:border-slate-700 transition-all shadow-md"
            >
              <ArrowLeft size={14} /> Back to Evaluations
            </Link>
            <div className="text-right">
              <span className="text-[10px] text-app-text-muted uppercase tracking-widest font-bold">Module</span>
              <p className="text-xs font-semibold text-app-accent">Vendor Master / Evaluation</p>
            </div>
          </div>

          {/* Vendor Summary Card */}
          <div className="bg-gradient-to-r from-slate-900 to-slate-900/60 border border-app-border rounded-2xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <div className="flex items-center gap-3">
                <span className="bg-app-accent/10 border border-app-accent/20 px-2.5 py-0.5 rounded-full text-xs font-bold text-app-accent">
                  {evalmarks?.vendorcode}
                </span>
              </div>
              <h1 className="text-xl md:text-2xl font-bold text-app-text mt-2 font-Poppins leading-tight">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="bg-app-surface-muted border border-app-border rounded-xl px-4 py-2.5 text-center min-w-[90px]">
                <span className="text-[10px] text-app-text-muted font-bold block uppercase tracking-wider">Fixed Score</span>
                <span className="text-base font-black text-app-accent mt-0.5 block">
                  {evalmarks?.finalfixedscore?.$numberDecimal || "0.00"}
                </span>
              </div>
              {evalmarks.finalscore2022 && (
                <div className="bg-app-surface-muted border border-app-border rounded-xl px-4 py-2.5 text-center min-w-[90px]">
                  <span className="text-[10px] text-app-text-muted font-bold block uppercase tracking-wider">Year 2022</span>
              ...
              {evalmarks.finalscore2023 && (
                <div className="bg-app-surface-muted border border-app-border rounded-xl px-4 py-2.5 text-center min-w-[90px]">
                  <span className="text-[10px] text-app-text-muted font-bold block uppercase tracking-wider">Year 2023</span>
              ...
              {evalmarks.finalscore2024 && (
                <div className="bg-app-surface-muted border border-app-border rounded-xl px-4 py-2.5 text-center min-w-[90px]">
                  <span className="text-[10px] text-app-text-muted font-bold block uppercase tracking-wider">Year 2024</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              {/* Subgroups Mapped */}
              {subgroups.length > 0 && (
                <div className="bg-app-surface border border-app-border rounded-2xl p-5 shadow-lg flex flex-col gap-4 animate-fadeIn">
                  <h2 className="text-sm font-bold uppercase tracking-wider text-app-text-muted flex items-center gap-1.5 border-b border-app-border pb-2">
              ...
              {/* Fixed Score Basis */}
              <div className="bg-app-surface border border-app-border rounded-2xl p-5 shadow-lg flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {evalmarks2.fixedevalyear1?.fixedeval?.map((fixed, index) => (
                    <div 
                      key={index} 
                      className="flex justify-between items-center bg-app-bg/50 border border-slate-850 rounded-xl p-3"
                    >
                      <span className="text-xs text-app-text-muted font-medium">{fixedscoretext[index]}</span>
                      <span className="text-sm font-bold text-app-text bg-app-surface border border-app-border w-8 h-8 rounded-full flex items-center justify-center">
                        {fixed}
                      </span>
                    </div>
                  ))}
                  ...
                  <div className="flex flex-wrap gap-4">
                    {evalmarks2.past.map((past, index) => 
                      past.pastyearscore > 0 ? (
                        <div 
                          key={index}
                          className="bg-app-bg/40 border border-slate-850 rounded-xl px-4 py-3 min-w-[120px] flex justify-between items-center flex-1"
                        >
                          <span className="text-xs text-app-text-muted font-bold uppercase">Year {past.pastyear}</span>
                          <span className="text-sm font-bold text-teal-400 bg-app-surface px-2 py-0.5 rounded border border-app-border">
                            {past.pastyearscore}
                          </span>
                        </div>
                      ) : null
                    ...
                    {["2022", "2023", "2024"].map((year, idx) => {
                      ...
                      return (
                        <div key={year} className="flex flex-col gap-2">
                          <span className="text-[10px] text-app-accent uppercase tracking-widest font-bold">
                            Transaction Data - Year {year}
                          </span>
                          <div className="overflow-x-auto w-full border border-slate-850 rounded-xl">
                            <table className="w-full text-left text-xs border-collapse">
                              <thead>
                                <tr className="bg-app-bg border-b border-slate-850 text-app-text-muted font-semibold uppercase tracking-wider text-[9px]">
                              ...
                              <tbody className="divide-y divide-slate-850">
                                {list.map((po, index) => (
                                  <tr key={index} className="hover:bg-slate-850/20 transition-colors">
                                    <td className="py-2 px-3 font-semibold text-app-text">{po.ponumber}</td>
                                    <td className="py-2 px-3 text-right text-app-text-secondary font-medium">
            ...
            <div className="lg:col-span-1 flex flex-col gap-4">
              <div className="bg-app-surface border border-app-border rounded-2xl p-5 shadow-lg flex flex-col gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-app-text-muted mt-1 leading-normal">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="bg-app-bg/50 border border-slate-850 hover:border-slate-800 rounded-xl p-4 flex gap-3 transition-colors relative overflow-hidden group">
                    ...
                    <div className="w-8 h-8 rounded-lg bg-app-accent/10 border border-app-accent/20 flex items-center justify-center text-app-accent shrink-0 mt-0.5">
                      <Cpu size={16} />
                    </div>
                  ...
                  <div className="bg-app-bg/50 border border-slate-850 hover:border-slate-800 rounded-xl p-4 flex gap-3 transition-colors relative overflow-hidden group">
                    ...
                    <div className="min-w-0 pr-12">
                      <h3 className="text-xs font-bold text-app-text group-hover:text-violet-400 transition-colors">
                        Procurement Team
                      </h3>
                      <p className="text-[10px] text-app-text-muted mt-1 leading-relaxed">
                  ...
                  <div className="bg-app-bg/50 border border-slate-850 hover:border-slate-800 rounded-xl p-4 flex gap-3 transition-colors relative overflow-hidden group">
                    ...
                    <div className="min-w-0 pr-12">
                      <h3 className="text-xs font-bold text-app-text group-hover:text-emerald-400 transition-colors">
                        Compliance & Safety
                      </h3>
                      <p className="text-[10px] text-app-text-muted mt-1 leading-relaxed">
...
function LoaderSpinner({ label }) {
  return (
    <div className="text-center p-6 flex flex-col items-center justify-center gap-3">
      ...
      <span className="text-xs text-app-text-muted font-semibold">{label}</span>
    </div>
  );
}
