...
const AllPurchasesReport = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const url = selectedYear === "all"
          ? "/api/reports/all-purchases?year=all"
          : `/api/reports/all-purchases?year=${selectedYear}`;
  ...
  const downloadExcel = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = `All_Purchases_Report_${selectedYear === "all" ? "all" : selectedYear}_${moment().format("YYYY-MM-DD_HH-mm")}.xlsx`;
      a.click();
  ...
  const openMonthwiseModal = () => {
    ...
    fetch(`/api/reports/all-purchases/monthwise?year=${selectedYear}`)
      .then((res) => {
  ...
  const detailsUrl = (row) => {
    ...
    return `/all-purchases-report/details?materialKey=${encodeURIComponent(row.materialKey)}&year=${encodeURIComponent(year)}`;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>All Purchases Report | MM Portal</title>
        <meta name="description" content="All purchase orders (31, 45, 46, 47, 61, 71 series) — materials by frequency and value, year-wise" />
      </Head>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">All Purchases Report</h1>
          <p className="text-gray-600 mt-2">
            Materials purchased most frequently (number of POs) and total value — all purchase orders (31, 45, 46, 47, 61, 71 series, etc). Year: 1 Jan – 31 Dec.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className="text-sm text-gray-500">
              {isAllYears ? "All POs in collection" : selectedYear != null ? `1 Jan ${selectedYear} – 31 Dec ${selectedYear}` : ""}
            </span>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase">
                        <button type="button" onClick={() => handleSort("totalQty")} className="ml-auto flex items-center gap-1 hover:text-gray-900">
                          Total qty (all POs)
                    ...
                    <tr className="bg-gray-50 border-t border-gray-200">
                      ...
                      <th colSpan={4} className="px-2 py-2 text-left text-xs text-gray-500">
                    ...
                    {displayData.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-gray-500">
                          {data.length === 0
                            ? (isAllYears ? "No PO data in the collection." : "No PO data for the selected year.")
                      ...
                      displayData.map((row, idx) => (
                        <tr key={row.materialKey ?? idx} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-right font-medium text-gray-900">{formatQuantity(row.totalQty)}</td>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-100">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase"># of POs</th>
                          ...
                          <tbody className="divide-y divide-gray-200">
                            {monthwiseData.length === 0 ? (
                              <tr>
                                <td colSpan={6} className="px-4 py-8 text-center text-gray-500">No month-wise data.</td>
                              </tr>
                            ) : (
