...
/**
 * GET /api/reports/import-purchases/monthwise?year=2024
 * Returns same columns as main report but one row per material per month (46-series, year required).
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const pipeline46 = [
      { $match: { ...match46, "po-date": { $exists: true, $nin: [null, ""] } } },
      ...dateFilterStage,
      materialGroupStage,
      {
        $group: {
    ...
    console.error("import-purchases monthwise API error:", error);
