...
/**
 * GET /api/reports/import-purchases?year=2024  (or ?year=all for entire collection)
 * Returns 46-series PO materials: for each material (by code or description),
 * number of distinct POs, total value, total qty (46* only), total qty (all PO types).
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const match46 = { "po-number": { $regex: /^46/ } };
    const pipeline46 = [
      { $match: allYears ? match46 : { ...match46, "po-date": { $exists: true, $nin: [null, ""] } } },
      ...dateFilterStage,
      materialGroupStage,
      {
        $group: {
    ...
    const [result46, resultAll] = await Promise.all([
      coll.aggregate(pipeline46).toArray(),
    ...
    const merged = result46.map((row) => ({
      ...row,
      totalQtyCash: row.totalQtyImport,
      totalQtyAll: allQtyByKey[row.materialKey] ?? 0,
    }));

    return res.json(merged);
  } catch (error) {
    console.error("import-purchases API error:", error);
