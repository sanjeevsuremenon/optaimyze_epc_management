...
/**
 * GET /api/reports/services-purchases/monthwise?year=2024
 * Returns same columns as main report but one row per material per month (61-series, year required).
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const pipeline61 = [
      { $match: { ...match61, "po-date": { $exists: true, $nin: [null, ""] } } },
      ...dateFilterStage,
      materialGroupStage,
      {
        $group: {
    ...
    console.error("services-purchases monthwise API error:", error);
