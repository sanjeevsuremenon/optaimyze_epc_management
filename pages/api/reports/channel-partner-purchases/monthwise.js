...
/**
 * GET /api/reports/channel-partner-purchases/monthwise?year=2024
 * Returns same columns as main report but one row per material per month (71-series, year required).
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const pipeline71 = [
      { $match: { ...match71, "po-date": { $exists: true, $nin: [null, ""] } } },
      ...dateFilterStage,
      materialGroupStage,
      {
        $group: {
    ...
    const [result71, resultAll] = await Promise.all([
    ...
    console.error("channel-partner-purchases monthwise API error:", error);
