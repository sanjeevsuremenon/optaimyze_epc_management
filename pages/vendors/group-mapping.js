    ...
    <div className="app-page min-h-screen flex flex-col pb-12">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="bg-app-surface/80 border border-app-border rounded-2xl shadow-lg p-6 mb-8 relative">
          ...
          {vendors.length > 0 && (
            <div className="absolute z-50 w-full left-0 mt-2 app-card rounded-xl shadow-2xl max-h-60 overflow-y-auto">
              {vendors.map((vendor) => (
                <div
          ...
          {searchTerm.length >= 3 && vendors.length === 0 && !isSearching && !selectedVendor && (
            <div className="absolute z-50 w-full left-0 mt-2 app-card rounded-xl shadow-2xl p-4 text-center text-app-text-muted text-sm">
          ...
          <div className="mt-8">
            <div className="mb-6 p-5 bg-cyan-950/30 border border-cyan-900/50 rounded-2xl shadow-sm">
              <h3 className="text-lg font-bold text-app-text mb-2">
                Selected Vendor: <span className="text-app-accent">{selectedVendor.vendorname}</span>
              </h3>
              <div className="flex flex-col sm:flex-row gap-4 text-sm text-app-text-secondary">
                <div className="flex items-center">
                  <span className="text-app-text-muted mr-2 uppercase text-xs font-bold tracking-wider">Code:</span>
                  <span className="font-mono bg-app-surface px-2 py-1 rounded border border-app-border">{selectedVendor.vendorcode}</span>
                ...
                <div className="flex items-center">
                  <span className="text-app-text-muted mr-2 uppercase text-xs font-bold tracking-wider">Source:</span>
                  <span className="bg-app-surface px-2 py-1 rounded border border-app-border">{selectedVendor.source}</span>
                </div>
              </div>
            </div>
        ...
        {!selectedVendor && (
          <div className="bg-app-surface-muted border border-app-border border-dashed rounded-2xl p-12 text-center mt-8">
            ...
            <h3 className="text-lg font-medium text-app-text-secondary">Select a Vendor to Map</h3>
