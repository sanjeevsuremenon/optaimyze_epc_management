    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col font-sans">
        <div className="flex-1 flex items-center justify-center">
          ...
          <span className="ml-4 text-slate-300 font-medium">Loading PO details...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 flex-1 flex flex-col font-sans">
      ...
      <main className="w-full max-w-full px-4 py-8">
        <div className="mx-auto" style={{ maxWidth: '1400px' }}>
          ...
          <Link href="/purchases-report" className="inline-flex items-center text-cyan-400 hover:text-cyan-300 mb-4 transition-colors font-semibold text-sm">
            <FiArrowLeft className="mr-2" /> Back to {backLabel}
          </Link>

          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-extrabold text-slate-100 tracking-tight mb-2">Material PO Details ({title})</h1>
            <div className="flex items-center gap-2 text-slate-400">
              <span className="font-semibold text-slate-300">Material:</span> <span className="font-mono bg-slate-800 px-2 py-0.5 rounded">{materialKey}</span>
              <span className="mx-2 text-slate-600">|</span>
              <span className="font-semibold text-slate-300">Period:</span> <span>{year === "all" ? "All Years" : year}</span>
              <span className="mx-2 text-slate-600">|</span>
              <span className="font-semibold text-slate-300">Total POs:</span> <span className="text-cyan-400 font-bold">{data.length}</span>
            </div>
          </div>
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-950 border-b border-slate-800">
                    <tr>
                      ...
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">PO Date</th>
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Vendor Code</th>
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Vendor Name</th>
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Unit Rate</th>
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Qty</th>
                      <th className="px-5 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Value (SAR)</th>
                    ...
                    {data.length === 0 ? (
                      ...
                      data.map((po, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/50 transition-colors group">
                          <td className="px-5 py-3 whitespace-nowrap text-sm font-bold text-cyan-400">
                            {po.poNumber}
                          </td>
                          <td className="px-5 py-3 whitespace-nowrap text-sm text-slate-300">
                            {po.poDate ? moment(po.poDate).format("MMM D, YYYY") : "—"}
                          </td>
                          <td className="px-5 py-3 whitespace-nowrap text-sm font-mono text-slate-400">
                            {po.vendorCode || "—"}
                          </td>
                          <td className="px-5 py-3 text-sm text-slate-200 capitalize font-medium break-words">
                            {po.vendorName?.toLowerCase() || "—"}
                          </td>
                          <td className="px-5 py-3 whitespace-nowrap text-sm font-mono text-slate-300 text-right">
