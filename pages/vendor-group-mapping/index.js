        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg p-6 mb-8 relative">
            <div className="relative">
              <label htmlFor="groupSearch" className="block text-xs font-semibold text-cyan-400 mb-2 uppercase tracking-wider">
              ...
              <div className="relative">
                <input
                  ...
                  disabled={loading}
                  className="w-full px-5 py-3.5 bg-slate-950 border border-slate-700 rounded-xl focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-slate-200 placeholder-slate-600 transition-colors shadow-inner disabled:opacity-50"
                />
                {searchFilter && (
                  <button
              ...
              {showDropdown && searchFilter.length >= 3 && filteredOptions.length === 0 && (
                <div className="absolute z-50 w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-4 text-center text-slate-400 text-sm">
            ...
            {selectedOption && (
              <div className="mt-6 p-4 bg-cyan-950/30 rounded-xl border border-cyan-900/50">
                ...
                <p className="text-slate-300">
                  <span className="font-semibold text-white">{selectedOption.groupName}</span> → 
                  <span className="font-semibold text-white"> {selectedOption.subgroupName}</span>
                  <span className="ml-3 inline-flex items-center px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wide uppercase bg-slate-900 text-cyan-400 border border-slate-700">
          ...
          {/* Loading State */}
          {loadingVendors && (
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg p-12 text-center">
              <div className="inline-block animate-spin rounded-full h-8 w-8 border-2 border-cyan-500 border-t-transparent"></div>
              <p className="mt-4 text-slate-400 font-medium">Loading vendors...</p>
            </div>
          )}
          ...
          {!loadingVendors && selectedOption && (
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg overflow-hidden">
              <div className="px-6 py-5 bg-slate-800/50 border-b border-slate-700 flex items-center justify-between">
                <div>
                ...
                <span className="px-3 py-1 bg-slate-950 text-emerald-400 rounded-full text-sm font-bold border border-slate-800">
              ...
              {vendors.length === 0 ? (
                <div className="p-12 text-center text-slate-500">
                  ...
                  <h3 className="text-lg font-medium text-slate-300">No vendors found</h3>
                  ...
                  <table className="min-w-full divide-y divide-slate-800">
                    <thead className="bg-slate-800/80">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Contact Person</th>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right">
                            {!vendor.isUnregistered && (
                              <button
                                ...
                                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-cyan-400 hover:text-cyan-300 rounded-lg text-xs font-semibold transition-colors border border-slate-700"
                              >
                                View Details
                              </button>
                            )}
          ...
          {!selectedOption && !loading && (
            <div className="bg-slate-900/50 border border-slate-800 border-dashed rounded-2xl p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-300">Select a Group</h3>
