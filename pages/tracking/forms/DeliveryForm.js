    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold flex items-center gap-2 text-gray-900 dark:text-app-text">
            <FiTruck className="text-orange-500" /> New Delivery Log
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-app-text-secondary">
            <FiX size={24} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-app-text ${errors.project ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
              required
              autoComplete="off"
              placeholder="Type to search projects..."
              onFocus={() => {
                if (projectInput.length >= 5 && projectSuggestions.length > 0) setShowSuggestions(true);
              }}
            />
          ...
          <div>
            ...
            <input type="text" name="title" value={formData.title} onChange={e => setFormData(prev => ({ ...prev, title: e.target.value }))} className={`w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-app-text ${errors.title ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`} required />
            {errors.title && <p className="mt-1 text-sm text-red-600">{errors.title}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Priority *</label>
            <select name="priority" value={formData.priority} onChange={e => setFormData(prev => ({ ...prev, priority: e.target.value }))} className={`w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-app-text ${errors.priority ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`} required>
          ...
          <div>
            ...
            <input type="text" name="deliveryReference" value={formData.deliveryReference} onChange={e => setFormData(prev => ({ ...prev, deliveryReference: e.target.value }))} className={`w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-app-text ${errors.deliveryReference ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`} required />
          ...
          <div>
            ...
            <textarea name="requestInfo" value={formData.requestInfo} onChange={e => setFormData(prev => ({ ...prev, requestInfo: e.target.value }))} className={`w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-app-text ${errors.requestInfo ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`} rows={3} required />
            {errors.requestInfo && <p className="mt-1 text-sm text-red-600">{errors.requestInfo}</p>}
          </div>
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
            <button type="button" onClick={onClose} className="px-6 py-2 border rounded-lg text-gray-700 dark:text-app-text-secondary hover:bg-gray-50 dark:hover:bg-gray-700">Cancel</button>
